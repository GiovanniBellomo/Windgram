...
| 2026-07-23 | **E3 spezzato in 4 sotto-passi (E3a–E3d)** invece di un unico refactor del renderer | E3 (far consumare il contratto a build_svg/build_chart) e' il passo piu' delicato (~500 righe che ricalcolano fisica). Spezzarlo tiene ogni commit piccolo e a golden invariato. Dettaglio in `REFACTOR.md`. |
| 2026-07-24 | **E3d — firma finale `build_svg(forecast)` / `build_chart(forecast, geom)`**: gli array numpy sciolti si RICOSTRUISCONO dal contratto in testa a ciascuna funzione (helper `_narr`, None→NaN) invece di riscrivere il codice di disegno | Ricostruire gli array mantiene invariato tutto il rendering a valle → rischio minimo e golden SVG byte-identico. Alternativa (riscrivere ogni accesso per indicizzare `forecast.hours[j]`) sarebbe stata molto piu' invasiva senza vantaggi. |
| 2026-07-24 | Le **stringhe di intestazione** (data IT, `period_str`, etichetta modello, `run_label`, orari corsa/generazione) le DERIVA il renderer dai metadati del contratto, non le passa piu' `main()` | Sono presentazione, non fisica: derivarle dal contratto e' l'unico modo per avere una vera firma `render(forecast)`. Costanti `_MONTHS/_WD/_MODEL_LABELS` portate a livello modulo in `windgram_v2.py`. |
| 2026-07-24 | Golden `tests/golden/forecast.json` corretto in un campo: `generated_utc` **17:09 → 17:00** UTC | L'harness congelava `GEN_TIME_STR="19:00"` e `GEN_UTC="17:09"` in modo indipendente e incoerente (17:09 UTC = 19:**09** a Roma). Da E3d il renderer deriva l'orario mostrato da `generated_utc`, quindi il metadato dev'essere auto-coerente col "19:00" del golden SVG (Roma=UTC+2). L'SVG golden e' rimasto byte-identico. |
| 2026-07-24 | **Refactoring a strati CONCLUSO** (Fasi A–G). Layout finale: `windgram/{sources,core,contract,render/{svg,json_api},cli}` + lanciatore sottile `windgram_v2.py`. Shim `windgram_arome.py` **rimosso** (G1a). | Obiettivo raggiunto: strati netti a dipendenza unidirezionale, contratto come confine, renderer senza fisica, superfici JSON/log abilitate. Ogni passo a golden invariato (rete di sicurezza `tools/snapshot.py`). Dettaglio in `REFACTOR.md`. |
| 2026-07-24 | `windgram/render/json_api.py` come superficie payload separata da `Forecast.to_json` (F1); log storico `history/` via `write_history` in `cli.py` (F2) | Separare il punto d'ingresso del payload dal metodo del contratto permette di evolvere la forma dell'API (envelope, compatto/indentato, header HTTP) senza toccare il dataclass. Il log storico a costo ~zero abilita la futura correzione statistica (previsione vs osservazioni). |
